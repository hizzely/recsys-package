...
def _select_random_events() -> set:
    return set(random.choices(
        ['REACT', 'COMMENT', 'SHARE'], weights=[3, 2, 1], k=random.choice([1, 2, 3])
    ))


def _select_random_article_ids(article_ids: list) -> set:
    return set(random.choices(
...
def generate_interactions(article_ids: list, user_count: int) -> list:
    ...
    for user_id in range(1, user_count + 1):
        for article_id in _select_random_article_ids(article_ids):
            for event in _select_random_events():
                interactions.append({
                    'user_id': user_id,
